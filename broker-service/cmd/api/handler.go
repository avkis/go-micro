...
func (app *Config) logItemViaRabbit(w http.ResponseWriter, entry LogPayload) {
	err := app.puthToQueue(entry.Name, entry.Data)
	if err != nil {
		app.errorJSON(w, err)
		return
	}
...
func (app *Config) logItemViaRPC(w http.ResponseWriter, entry LogPayload) {
	...
	var payload jsonResponse
	payload.Error = false
	payload.Message = "Logged via RPC!" + result

	app.writeJSON(w, http.StatusAccepted, payload)
}

func (app *Config) puthToQueue(name, msg string) error {
...
func (app *Config) sendMail(w http.ResponseWriter, mail MailPayload) {
	// create some json we'll send to the mailer service
	...
	var payload jsonResponse
	payload.Error = false
	payload.Message = "Email sent to: " + mail.To
